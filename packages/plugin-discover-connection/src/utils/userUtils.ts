 ...
 */
export async function getUsernameFromMemories(runtime: IAgentRuntime, userId: string): Promise<string | undefined> {
    try {
 ...
 */
export async function updateEntityWithUsername(runtime: IAgentRuntime, userId: string): Promise<void> {
    try {
        const entity = await runtime.getEntityById(userId);
        if (!entity) {
        ...
        logger.error(`[discover-connection] Error updating entity with username for ${userId}: ${error}`);
    }
}
 ...
 */
export async function getUserInfo(runtime: IAgentRuntime, userId: string): Promise<{
    displayName: string;
    username?: string;
}> {
    try {
        ...
        if (!username) {
            ...
            if (username) {
                ...
                updateEntityWithUsername(runtime, userId).catch(error => {
                    logger.error(`[discover-connection] Background entity update failed for ${userId}: ${error}`);
                });
            }
        }
